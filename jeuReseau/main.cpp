...
{
    /**Initialisation des différentes bibliothèques SDL**/
    TTF_Init();
    ...
    /**Libération des police et fermeture des bibliothèque**/
...
void jouer(SDL_Surface *screen, int vieClefOr[3])
{
    ...
    /**Initialisation de la position du joueur pour la première fois**/
...
void deplacer_personnage(int carte[NB_BLOCS_LARGEUR][NB_BLOCS_HAUTEUR], SDL_Rect *position, int direction, int vieClefOr[3])
{
    switch(direction) /**y-1*/
    {
      /**/case HAUT:
            if(position->y - 1 < 0)
            {
                /**On arrête ici si la case suivante sort du tableau**/
            }
            else
            {
                ...
                else
                {
                    ...
                    {
                        /**Arrêt si la case suivante est un rocher**/
                    }
                    ...
                    else if(carte[position->x][position->y - 1] == COFFRE)
                    {
                        ...
                        {
                            /**On arrête si on n'a pas de clef pour ouvrir le coffre**/
                        }
                    }
                }
            }
...
void afficher_ecran_de_fin(SDL_Surface *screen, int vieClefOr[3])
{
    ...
    else if(vieClefOr[2] >= 10) //Si le joueur à plus de 10 pièces d'or
    {
